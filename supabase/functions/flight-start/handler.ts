...
export interface FlightStartEnvironment {
  SUPABASE_URL?: string;
  SUPABASE_ANON_KEY?: string;
  SUPABASE_SERVICE_ROLE_KEY?: string;
}

interface FlightStartRequest {
  dispatchId: string;
...
function parseRequestBody(rawBody: string): FlightStartRequest {
  let value: unknown;
  try {
  ...
  if (keys.length !== 2 || keys[0] !== "dispatchId" || keys[1] !== "idempotencyKey") {
...
function readRuntimeConfiguration(environment: FlightStartEnvironment): {
  anonKey: string;
  ...
  if (
    ...
    throw new HttpError(503, "configuration_unavailable", "Flight start is not configured.");
  }
...
function isFlightStartResponse(value: unknown): value is FlightStartResponse {
  if (value === null || typeof value !== "object" || Array.isArray(value)) {
    return false;
  }
  const response = value as Partial<FlightStartResponse>;
  return (
    response.schemaVersion === 1 &&
    response.state === "active" &&
    UUID_PATTERN.test(response.aircraftId ?? "") &&
    UUID_PATTERN.test(response.dispatchId ?? "") &&
    TIMESTAMP_PATTERN.test(response.startedAt ?? "") &&
    !Number.isNaN(Date.parse(response.startedAt ?? ""))
  );
}
...
async function startFlight(
  ...
  request: FlightStartRequest,
): Promise<FlightStartResponse> {
  let response: Response;
  try {
    response = await fetchImplementation(`${configuration.supabaseUrl}/rest/v1/rpc/start_flight_from_dispatch`, {
      method: "POST",
      headers: {
    ...
    throw new HttpError(503, "flight_start_unavailable", "Flight start is unavailable.");
  }
  ...
  try {
    ...
    throw new HttpError(502, "invalid_backend_response", "Flight start returned an invalid response.");
  }
  if (!isFlightStartResponse(value) || value.dispatchId !== request.dispatchId) {
    throw new HttpError(502, "invalid_backend_response", "Flight start returned an invalid response.");
  }
  return {
...
export function createFlightStartHandler(
  environment: FlightStartEnvironment,
  ...
  return async (request: Request): Promise<Response> => {
    ...
    try {
      ...
      const flightStartRequest = parseRequestBody(rawBody);
      ...
      const result = await startFlight(fetchImplementation, configuration, user, flightStartRequest);
      return jsonResponse(200, result);
    } catch (error) {
      if (error instanceof HttpError) {
        return jsonResponse(error.status, { error: { code: error.code, message: error.message } });
      }
      return jsonResponse(500, { error: { code: "internal_error", message: "Flight start failed." } });
    }
  };
}
