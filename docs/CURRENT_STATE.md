...
deux conditions de sortie de `Draft` étaient satisfaites et son implémentation est
`Review` sur `feature/T0051-authoritative-flight-settlement`, non fusionnée. Le
prochain ticket de ce flux est l'endpoint authentifié de la clôture, encore à
ouvrir, comme celui du démarrage. Le flux desktop a livré T0052 puis T0053 dans
`main` ; son prochain ticket n'est pas encore ouvert. Le flux moteur de vol et
bridge a livré T0054 ; son prochain ticket est T0059, `Draft` faute du prérequis
physique MSFS 2024 et SDK SimConnect. Les transverses T0055 et T0056 sont `Ready`.
La location T0032 reste bloquée sur ses décisions produit et la persistance Windows
