...
| KI-023 | Medium | Governance / Quality | La constante `SOURCES` des deux workflows de la boucle ordonne à chaque agent de lire l'intégralité des sources de vérité, sans égard à son rôle : l'agent de sélection ne peut rien décider hors du sélecteur déterministe et lit pourtant `docs/CURRENT_STATE.md` en entier. Le coût domine d'environ deux ordres de grandeur celui des charges JSON corrigé par T0064. | Mesure du 5 août 2026 sur `origin/main` `c0f16dc` : `AGENTS.md` 21 117 caractères, `docs/CURRENT_STATE.md` 66 714, `docs/ROADMAP.md` 9 535, soit environ 24 000 tokens de lecture imposée par agent, plus `docs/QUALITY.md` 51 328 pour les commandes de validation ; `ticket-run.js` injecte `SOURCES` dans huit agents ou plus par vague | Décision d'Andy sur l'ordre de lecture d'`AGENTS.md`, section Sources de vérité | Open |
