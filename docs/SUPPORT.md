...
Le bridge .NET doit être livré avec son runtime. **Pour utiliser Thrustline,
l'utilisateur n'a rien à installer** : ni SDK .NET, ni outils de développement, ni
SDK MSFS.
...
- la personne désigne une copie qu'elle possède déjà par le chemin explicite
  `--simconnect-library`.

Tout le reste de l'application ne dépend pas de cela et reste utilisable :
compagnie, catalogue, achat, dispatch, flotte, départ et clôture. MSFS 2024
